...
#define BLYNK_TEMPLATE_ID "TMPL2CpAdY1vo"
#define BLYNK_TEMPLATE_NAME "Air Quality Monitoring"
#define BLYNK_AUTH_TOKEN "_gLas1nGHYVus_xbzWhmtCrMq3938MJH"
#define BLYNK_PRINT Serial
#include <BlynkSimpleEsp8266.h>
String apiKey = "KH8FBZJU2M9DL7EA";  // API key from ThingSpeak
...
// WiFi parameters
//char ssid[] = "Abdul";  // Enter your Wifi Username
//char pass[] = "sowsow123";  // Enter your Wifi password
//char ssid[] = "CANALBOX-AAAF";  // Enter your Wifi Username
//char pass[] = "2igjEUBLy5";  // Enter your Wifi password

char ssid[] = "iPhone";  // Enter your Wifi Username
char pass[] = "zulumetang";  // Enter your Wifi password
...
#define AIO_USERNAME    "Dourasow"
#define AIO_KEY         "aio_hBsA04Rtuo5AAQr5iLB11KJkrcOa"
...
LiquidCrystal lcd(D6, D5, D0, D2, D3, D4);
...
if(! mqtt.ping(3)) {
  ...
  if (! humidity.publish(humidity_data))
   ...
   if (! c02.publish(val1))
    Serial.println(F("Failed to publish C02"));
  else
    Serial.println(F("C02 published!"));

     if (! c0.publish(val))
    Serial.println(F("Failed to publish C0"));
  else
    Serial.println(F("C0 published!"));

     if (! nh3.publish(val2))
    Serial.println(F("Failed to publish NH3"));
  else
    Serial.println(F("NH3 published!"));


 String message = "";

  // Check CO2
  if (val1 > 1000) { // Threshold for CO2
    message += "CO2 levels high: " + String(val1) + " ppm\n";
  }

  // Check CO
  if (val > 35) { // Threshold for CO
    message += "CO levels dangerous: " + String(val) + " ppm\n";
  }

  // Check NH3
  if (val2 > 25) { // Threshold for NH3
    message += "NH3 levels exceed safe limit: " + String(NH3) + " ppm";
  }

  // Send notification if any thresholds were exceeded
  if (!message.isEmpty()) {
   // Blynk.email("abdulsow655@gmail.com.com", "Air Quality Alert!", message);
